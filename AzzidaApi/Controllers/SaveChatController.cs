...
{
    public class SaveChatController : ApiController
    {
        ...
        public IHttpActionResult Post(int Id, int ToId, int FromId, string IsTyping,string UserMessage,string MessageDateTime, int JobId)
        {
            string retVal = string.Empty;
            MasterUpdate objmaster = new MasterUpdate();


            var obj = objmaster.SaveChat(Id, ToId, FromId, IsTyping, UserMessage, MessageDateTime, JobId);
            if (obj != null)
            {
                try
                {
