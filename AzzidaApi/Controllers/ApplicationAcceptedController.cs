...
{
    public class ApplicationAcceptedController : ApiController
    {
      //  string path = System.Configuration.ConfigurationManager.AppSettings["ImagePath"];
        ...
        public IHttpActionResult Post(int JobId, int SeekerId, int ListerId, string IsAcceptedByLister)
        {
            string retVal = string.Empty;
            MasterUpdate objmaster = new MasterUpdate();


            var obj = objmaster.ApplicationAccepted(JobId, SeekerId, ListerId, IsAcceptedByLister);
            if (obj != null)
            {
                try
                {
